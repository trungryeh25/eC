...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        is_existing_cart_item = CartItem.objects.filter(product=product, user=current_user).exists()
        if is_existing_cart_item:
        ...
        is_existing_cart_item = CartItem.objects.filter(product=product, cart=cart).exists()
        if is_existing_cart_item:
            cart_items = CartItem.objects.filter(
                product=product,
                cart=cart
